...
bool collisionC::RectInRect(RECT rtDesk, RECT rtSrc)
{
	POINT cDesk, cSrc;
...
bool collisionC::RectInPt(RECT rtDesk, POINT pt)
{
	if ((rtDesk.left <= pt.x && rtDesk.right >= pt.x) &&
		(rtDesk.top <= pt.y && rtDesk.bottom >= pt.y)) {
		return true;
	}
	return false;
}

bool collisionC::SphereInSphere(RECT rtDesk, RECT rtSrc)
{
...
bool collisionC::SphereInPt(RECT rtDesk, POINT pt)
{
	sphere sphereDesk;
	sphereDesk.pCenter.x = (rtDesk.left + rtDesk.right) / 2;
	sphereDesk.pCenter.y = (rtDesk.top + rtDesk.bottom) / 2;
